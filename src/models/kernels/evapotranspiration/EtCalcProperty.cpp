...
//############################################################*
double et::calculate_net_radiation_W_per_sq_m
        (
    ...
    outgoing_longwave_radiation_W_per_sq_m =
            surf_rad_params->surface_longwave_emissivity * stefan_boltzmann_constant *
    ...
    if (et_options->yes_aorc == FALSE)  // we must calculate longwave incoming from the atmosphere
        saturation_water_vapor_partial_pressure_Pa = calc_air_saturation_vapor_pressure_Pa(
    ...
    if (et_options->yes_aorc == FALSE) {
        ...
        } else  // not overcast, use TVA (1972) formulation, taken from Bras R.L., textbook, pg. 44.
        {
            ...
            atmosphere_longwave_emissivity =
                    0.740 + 0.0049 * actual_water_vapor_partial_pressure_Pa / 100.0; //conv. Pa to mb
            ...
            surf_rad_forcing->incoming_longwave_radiation_W_per_sq_m = atmosphere_longwave_emissivity * Klw *
                                                                       ...
                                                                       pow(surf_rad_forcing->air_temperature_C + TK,
                                                                           4.0);
        }
    }
...
// F.L. Ogden, NOAA National Weather Service, 2020            *
//############################################################*
double et::calculate_aerodynamic_resistance
        (
    ...
    // input sanity checks.
    if (1.0e-06 >= wind_speed_measurement_height_m)
        wind_speed_measurement_height_m = 2.0;  // standard measurement height
    if (1.0e-06 >= humidity_measurement_height_m)
        humidity_measurement_height_m = 2.0;  // standard measurement height
    if (1.0e-06 >= momentum_transfer_roughness_length_m)
        fprintf(stderr,
                "momentum_transfer_roughness_length_m is tiny in calculate_aerodynamic_resistance().  Should not be tiny.\n");
    if (1.0e-06 >= heat_transfer_roughness_length_m)  //warn.  Should not be tiny.
        fprintf(stderr,
                "heat_transfer_roughness_length_m is tiny in calculate_aerodynamic_resistance().  Should not be tiny.\n");
    ...
    ra = log((zm - d) / zom) * log((zh - d) / zoh) /
         (von_karman_constant_squared * uz);  // this is the equation for the aero. resist.
...
// F.L. Ogden, NOAA National Weather Service, 2020            *
//############################################################*
double et::calc_air_saturation_vapor_pressure_Pa(double air_temperature_C) {
    double air_sat_vap_press_Pa =
            611.0 * exp(17.27 * air_temperature_C / (237.3 + air_temperature_C));  // it is 237.3
...
// F.L. Ogden, NOAA National Weather Service, 2020                 *
//#################################################################*
double et::calc_slope_of_air_saturation_vapor_pressure_Pa_per_C(double air_temperature_C) {
...
// F.L. Ogden, NOAA National Weather Service, 2020            *
//############################################################*
double et::calc_liquid_water_density_kg_per_m3(double water_temperature_C) {
    ...
    if (1000 < water_density_kg_per_m3)
        water_density_kg_per_m3 = 1000.0;   // this empirical function yield 1000.151 at 0C.
...
void et::calculate_solar_radiation(solar_radiation_options *options, solar_radiation_parameters *params,
    ...
    solar_declination_angle_degrees =
            23.45 * M_PI / 180.0 * cos(2.0 * M_PI / 365.0 * (172.0 - forcing->day_of_year));
    ...
    M = 2.0 * M_PI * forcing->day_of_year /
        365.242;     // mean anomaly of sun  from wikipedia, works for leap years
    equation_of_time_minutes =
            -7.655 * sin(M) + 9.873 * sin(2.0 * M + 3.588);    // an approximation of the equation of time, minutes
    zulu_time_h = forcing->zulu_time_h -
                  equation_of_time_minutes / 1440.0; // adjust the position of the sun for analemma effect
    ...
    if (alpha > 0.0)  // the sun is over the horizon
    {
        ...
        Io = solar_constant_W_per_sq_m /
             (r * r);   //  J/(m^2 s) at top of atmosphere, adjusted for orbital eccentricity

        optical_air_mass =
                (1.002432 * pow(sinalpha, 2.0) + 0.148386 * sinalpha + 0.0096467) /         // after Young 1994
        ...
        fh1 = exp(-1.0 * params->site_elevation_m /
                  8000.0);  // elev. in meters, effect of atmos. thickness on air mass
        b = 0.664 + 0.163 / fh1;

        // note, atm_turbidity is equal to Tlk in Ineichen and Perez, 2002.
        Ic = b * Io *
             exp(-0.09 * optical_air_mass * (forcing->atmospheric_turbidity_factor - 1.0));  // clear sky radiation
        ...
        if (options->cloud_base_height_known == TRUE) {
            ...
            kshort = 0.18 + 0.0853 * forcing->cloud_base_height_m /
                            1000.0;   // convert cloud base height to km for this calc.
            Ips = Ic * (1.0 - (1.0 - kshort) *
                              forcing->cloud_cover_fraction);   // insolation considering clouds, Eagleson, 1970.
        } else {
        ...
        results->solar_radiation_horizontal_cloudy_flux_W_per_sq_m =
                Ips * sinalpha;  // on a horizontal plane tangent to earth
...
void et::calculate_intermediate_variables
        (
    ...
    water_latent_heat_of_vaporization_J_per_kg =
            2.501e+06 - 2370.0 * et_forcing->water_temperature_C;  // eqn 2.7.6 Chow etal.
    // aka 'lambda'
    ...
    if ((1.0e-06 > et_params->heat_transfer_roughness_length_m) ||
        (1.0e-06 >
         et_params->momentum_transfer_roughness_length_m))   // zero should be passed down if these are unknown
    {
    ...
    if ((0.0 < et_forcing->relative_humidity_percent) && (100.0 >= et_forcing->relative_humidity_percent)) {
        ...
        air_actual_vapor_pressure_Pa =
                et_forcing->relative_humidity_percent / 100.0 * air_saturation_vapor_pressure_Pa;

        // calculate specific humidity, q_v
        et_forcing->specific_humidity_2m_kg_per_kg =
                0.622 * air_actual_vapor_pressure_Pa / et_forcing->air_pressure_Pa;
    } else {
        // if here, we must be using AORC forcing that provides specific humidity instead of relative humidity
        air_actual_vapor_pressure_Pa =
                et_forcing->specific_humidity_2m_kg_per_kg * et_forcing->air_pressure_Pa / 0.622;
    ...
    // DELTA
    slope_sat_vap_press_curve_Pa_s = calc_slope_of_air_saturation_vapor_pressure_Pa_per_C(
            et_forcing->air_temperature_C);
...
int et::is_fabs_less_than_eps(double a, double epsilon)  // returns true if fabs(a)<epsilon
{
